...
//
// Each rail is a hook in section: a flange that laps over the tray plate's rim,
// and a skirt that drops down the case side. Two M4 thumbscrews per rail press
// against the case; tightening squeezes the case between the two rails AND
// pins the plate down under the flanges. Nothing pierces the plate, nothing is
// glued to the instrument, and the whole thing lifts off when you back the
// screws out.
//
...
module rail() {
    difference() {
        union() {
            ...
            // thumbscrew bosses on the outer skirt face (more thread engagement)
            for (y = [-THUMB_Y, THUMB_Y])
                translate([SKIRT_OUT, y, THUMB_Z]) rotate([0, 90, 0])
                    cylinder(h = THUMB_EXT, d = THUMB_BOSS);
        }
        // tapped holes, right through boss + skirt
        for (y = [-THUMB_Y, THUMB_Y])
            translate([SKIRT_IN - 1, y, THUMB_Z]) rotate([0, 90, 0])
                cylinder(h = SKIRT_T + THUMB_EXT + 2, d = THUMB_TAP);
    }
}
