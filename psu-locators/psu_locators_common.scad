...
// WHEN BOTH WALLS EXIST THEY MUST OVERLAP, NOT MEET AT A POINT.
//
// Written as two cubes butted at the origin they touch along a single EDGE, and
// that edge ends up shared by four triangles — non-manifold. It only shows on
// locators where both walls are generated: the ENGINDOT has 1.50 mm of X margin
// against NUB_T 1.2 so it gets both and produced 4 bad edges, while the OWON has
// -0.65 and gets end walls only, so it rendered clean and hid the bug.
//
// Running the side wall up through the corner by NUB_T makes it a real L with
// volumetric overlap.
// The nubs also SINK into the plate rather than butting onto its top face. A nub
// standing exactly on z0 is a partial-area join at an exact plane, which leaves
// non-manifold edges along the nub's footprint — 2 of them survived the L fix
// above until the nubs were sunk. Exact-plane butting is for FULL-face joins.
NUB_SINK = 0.5;
module _corner(xw, yw) {
    if (xw) translate([0, -NUB_LEN, -NUB_SINK])
        cube([NUB_T, NUB_LEN + (yw ? NUB_T : 0), NUB_H + NUB_SINK]);   // side wall
    if (yw) translate([-NUB_LEN, 0, -NUB_SINK])
        cube([NUB_LEN, NUB_T, NUB_H + NUB_SINK]);                       // end wall
}
